// Perplexity adapter for prompt insertion and reading
window.promptLibraryAdapter = {
  ...
  match(url) {
    return url.hostname === 'www.perplexity.ai';
  },
  ...
  async insert(text) {
    ...
    if (!composer) {
      console.warn('Perplexity composer not found');
      return false;
    }
  ...
  async readCurrentInput() {
    ...
    const composer = this.findComposer();
    if (!composer) {
      console.error('Perplexity composer not found for reading');
    ...
    console.log('Perplexity: Read text length:', text.length, 'Preview:', text.substring(0, 100));
    return text;
  },
  ...
  checkLLMStatus() {
    // Check for indicators that Perplexity has finished responding
    // Look for stop button or loading indicators
    const stopButton = document.querySelector('button[aria-label*="Stop"]');
    const loadingIndicator = document.querySelector('[class*="loading"]');
    
    if ((!stopButton || !this.isVisible(stopButton)) && (!loadingIndicator || !this.isVisible(loadingIndicator))) {
  ...
  triggerEvents(element) {
    ...
    // Trigger focus and blur for better compatibility
